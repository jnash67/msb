...
public class CsvUtil implements Serializable {
    ...
    public static <T extends DisplayFriendly> List<T> csvWithHeaderToDisplayFriendlyList(Class<T> clazz,
                                                                                         String csvString, User u) {
        //        System.out.println("==========================");
    ...
    private static BeanInstructions getPlanBeanInstructions() {
        BeanInstructions bi = getListedPropertiesBeanInstructions(Plan.class, DisplayFriendly.propertyIdList(Plan
                .class));
    ...
    private static BeanInstructions getMedicalExpenseBeanInstructions(final User u) {
        BeanInstructions bi = getListedPropertiesBeanInstructions(MedicalExpense.class,
                DisplayFriendly.propertyIdList(MedicalExpense.class));
        bi.setDate("date", "dd-MM-yyyy");
        bi.setConverter("medicalExpenseType", new EasierAbstractConverter<MedicalExpense.MedicalExpenseType>
                (MedicalExpense.MedicalExpenseType.class) {
        ...
        bi.setConverter("prescriptionTierType", new EasierAbstractConverter<MedicalExpense.PrescriptionTierType>
                (MedicalExpense.PrescriptionTierType.class) {
    ...
    private static BeanInstructions getProviderBeanInstructions() {
        BeanInstructions bi = getListedPropertiesBeanInstructions(Provider.class,
                DisplayFriendly.propertyIdList(Provider
                        .class));
        bi.setConverter("providerType", new EasierAbstractConverter<Provider.ProviderType>(Provider.ProviderType
                .class) {
    ...
    static public <T extends DisplayFriendly> String displayFriendlyToCsv(Class<T> clazz, T entity,
                                                                          User u) {
    ...
    static public <T extends DisplayFriendly> String displayFriendlyListToCsv(Class<T> clazz, List<T> list,
                                                                              User u) {
    ...
    // switch to the below when csveed bean writing functionality works fully
    static public <T extends DisplayFriendly> String newDisplayFriendlyListToCsv(Class<T> clazz, List<T> list,
                                                                                 User u) {
    ...
    private static <T extends DisplayFriendly> List<T> readBeansTolerantly(Class<T> clazz, CsvClient<T> client) {
        ...
        while (!client.isFinished()) {
            try {
                ...
                exc.printStackTrace();
                System.out.println("CSV row for class " + clazz.getSimpleName() + " failed to parse.  Row will be " +
                        "ignored and permanently lost when class file is re-persisted.");
            }
        } return beans;
    }
