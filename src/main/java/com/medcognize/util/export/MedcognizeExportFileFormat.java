...
public class MedcognizeExportFileFormat {
    ...
    public static SerializableZipFile createFileFromUser(User u) {
        ...
        ZipUtil.writeZipEntry(zos, userZipEntryFileName, CsvUtil.displayFriendlyToCsv(User.class, u,
                u).getBytes(), blankPassword);
        ...
        ZipUtil.writeZipEntry(zos, fsasZipEntryFileName, CsvUtil.displayFriendlyListToCsv(Fsa.class, u.getFsas(),
                u).getBytes(), blankPassword);
        ZipUtil.writeZipEntry(zos, familyMembersZipEntryFileName, CsvUtil.displayFriendlyListToCsv(FamilyMember
                .class, u.getFamilyMembers(), u).getBytes(), blankPassword);
        ZipUtil.writeZipEntry(zos, providersZipEntryFileName, CsvUtil.displayFriendlyListToCsv(Provider.class,
                u.getProviders(), u).getBytes(), blankPassword);
        ZipUtil.writeZipEntry(zos, medicalExpensesZipEntryFileName, CsvUtil.displayFriendlyListToCsv
                (MedicalExpense.class, u.getMedicalExpenses(), u).getBytes(), blankPassword);
        try {
            zos.finish();
            zos.close();
    ...
    public static User createUserFromFile(SerializableZipFile zf) {
        try {
            ...
            FileHeader familyMembersZipEntry = ZipUtil.getZipEntryFileHeader(zf,
                    familyMembersZipEntryFileName);
            FileHeader providersZipEntry = ZipUtil.getZipEntryFileHeader(zf, providersZipEntryFileName);
            FileHeader medicalExpensesZipEntry = ZipUtil.getZipEntryFileHeader(zf,
                    medicalExpensesZipEntryFileName);
