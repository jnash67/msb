...
@DateFieldOnOrAfter(first = "fsaEndDate", second = "fsaStartDate", message = "The end date must be equal to or after the "
        + "start date")
@DateFieldInYear.List({@DateFieldInYear(dateField = "fsaStartDate", yearField = "fsaYear",
        message = "The start date is not in the year of the FSA"), @DateFieldInYear(dateField = "fsaEndDate",
        yearField = "fsaYear", message = "The end date is not in the year of the FSA")})
@Data
@NoArgsConstructor
...
public class Fsa extends DisplayFriendlyAbstractEntity implements Serializable {
    ...
    private static final String captionString = "fsaName:FSA Name, amountInFsa:Amount In FSA, " +
            "fsaYear:Year, fsaStartDate:Period Start, fsaEndDate:Period End";
